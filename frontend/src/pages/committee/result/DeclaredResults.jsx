...
const [unDeclarableResults] = useResultUnDeclarationsMutation()
  const { data, isLoading, refetch } = useGetAllProgramQuery();
        ...
        return (
          <div className="flex flex-row gap-2 absolute right-5">
            ...
            <button onClick={() => handleClick(row._id)}>
              <MdDeleteForever className={`text-red-500 hover:text-red-900 text-2xl`} />
            </button>
          </div>
        );
      },
  ...
  [ d.declaredOrder || "" , d.id || "" , d.name || "" , filterZone(d.zone) || "" , d.type || ""]
  .some((item) => String(item).toLowerCase().includes(filterText.toLowerCase()))
  )
  return (
    ...
    <p className="text-blue-300 animate-pulse">If you want to undeclare result, enter the delete button</p>
