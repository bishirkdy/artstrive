...
import { useViewStudentPointsByZoneMutation, useProgramCountQuery } from "../../redux/api/programApi";
...
const StudentScoreByZone = () => {
  ...
  const [fetchScores, { data, isLoading }] = useViewStudentPointsByZoneMutation();

  const { data: zoneData, isLoading: zoneIsLoading, isError, error } = useViewZoneQuery();
  ...
  const columns = [
    ...
    { name: "Student Id", selector: (row) => row.id, width: isMobile ? "100px" : "20%", wrap: true },
    { name: "Name", selector: (row) => row.name, width: isMobile ? "160px" : "30%", wrap: true },
    { name: "Team", selector: (row) => row.team, width: isMobile ? "120px" : "20%", wrap: true },
    { name: "Total Score", selector: (row) => row.totalScore || 0, width: isMobile ? "100px" : "20%", center: true },
  ];
  ...
  if (isLoading || zoneIsLoading || programCountLoading) {
    return (
  ...
  return (
    <div className="flex flex-col items-center mt-[16vh] h-[90vh] overflow-y-auto lg:ml-[20vw] overflow-x-hidden scrollbar-hide">
      ...
      <div className="flex items-center md:px-2 lg:px-0 justify-between w-[98vw] lg:w-[75vw]">
        <div>
          <h1 className="text-[white] hidden md:block font-bold text-2xl">View Students Score By Zone</h1>
        </div>
